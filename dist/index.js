...
4. Add your terminal/app to the allowed applications
5. Restart your terminal and try again`}}catch($){return{hasAccess:!1,message:`Error checking Calendar access: ${$ instanceof Error?$.message:String($)}`}}}async function h5(){let $=await m8(["list-calendars"]);return JSON.parse($).map((Q)=>Q.name)}async function x5($=10,J,Q,H){console.error("getEvents - Starting to fetch calendar events via EventKit");let Y=Math.min($,o0.MAX_EVENTS),W=["list-events","--limit",String(Y)];if(J)W.push("--from",J);if(Q)W.push("--to",Q);if(H)W.push("--calendar",H);let X=await m8(W),w=JSON.parse(X);return console.error(`getEvents - Found ${w.length} event(s)`),w}async function T5($,J=10,Q,H,Y){console.error(`searchEvents - Searching for: "${$}" via EventKit`);let W=Math.min(J,o0.MAX_EVENTS),X=["search-events","--query",$,"--limit",String(W)];if(Q)X.push("--from",Q);if(H)X.push("--to",H);if(Y)X.push("--calendar",Y);let w=await m8(X),z=JSON.parse(w);return console.error(`searchEvents - Found ${z.length} matching event(s)`),z}async function y5($,J,Q,H,Y,W=!1,X){try{let w=await u8();if(!w.hasAccess)return{success:!1,message:w.message};if(!$.trim())return{success:!1,message:"Event title cannot be empty"};if(!J||!Q)return{success:!1,message:"Start date and end date are required"};let z=new Date(J),q=new Date(Q);if(isNaN(z.getTime())||isNaN(q.getTime()))return{success:!1,message:"Invalid date format. Please use ISO format (YYYY-MM-DDTHH:mm:ss.sssZ)"};if(q<=z)return{success:!1,message:"End date must be after start date"};console.error(`createEvent - Attempting to create event: "${$}"`);let V=X||"Calendar",K=`
tell application "Calendar"
