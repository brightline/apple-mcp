...
[URL: ${j}]`;return U}))}catch(J){if(console.error("Error reading unread messages:",J),J instanceof Error)console.error("Error details:",J.message),console.error("Stack trace:",J.stack);return[]}}async function G5($,J,Q){let H=new Map,Y=Q.getTime()-Date.now();if(Y<0)throw Error("Cannot schedule message in the past");let W=setTimeout(async()=>{try{await c4($,J),H.delete(W)}catch(X){console.error("Failed to send scheduled message:",X)}},Y);return H.set(W,{phoneNumber:$,message:J,scheduledTime:Q,timeoutId:W}),{id:W,scheduledTime:Q,message:J,phoneNumber:$}}var a1,H5=3,W5=1000,K5;var h8=W0(()=>{U0();a1={MAX_MESSAGES:50,MAX_CONTENT_PREVIEW:300,TIMEOUT_MS:8000};K5={sendMessage:c4,readMessages:z5,scheduleMessage:G5,getUnreadMessages:q5,requestMessagesAccess:N8}});var y8={};S0(y8,{default:()=>k5});import{unlinkSync as V5}from"node:fs";function x8($){if(!$||$.trim()==="")return[];return $.split(/\r?\n/).filter((Q)=>Q.includes("\t")).map((Q)=>{let H=Q.split("\t");return{subject:H[0]||"No subject",sender:H[1]||"Unknown",dateSent:H[2]||"",content:H[3]||"",isRead:H[4]==="true",mailbox:H[5]||"Unknown"}})}async function j5(){try{return await f(`
...
end tell`,L0.TIMEOUT_MS);if(H&&typeof H==="string"&&H.includes("SUCCESS"))return[];return[]}catch(J){return console.error(`Error getting reminders: ${J instanceof Error?J.message:String(J)}`),[]}}async function n4($){try{let J=await E0();if(!J.hasAccess)throw Error(J.message);if(!$||$.trim()==="")return[];let H=await f(`
tell application "Reminders"
    try
...
end tell`,L0.TIMEOUT_MS);return[]}catch(Q){return console.error(`Error getting reminders from list by ID: ${Q instanceof Error?Q.message:String(Q)}`),[]}}var L0,E5;var l8=W0(()=>{U0();L0={MAX_REMINDERS:50,MAX_LISTS:20,TIMEOUT_MS:8000};E5={getAllLists:A5,getAllReminders:f5,searchReminders:n4,createReminder:P5,openReminder:b5,getRemindersFromListById:C5,requestRemindersAccess:E0}});var u8={};S0(u8,{default:()=>Z5});function o4($){if(!$||$.trim()==="")return[];return $.split(/\r?\n/).filter((Q)=>Q.includes("\t")).map((Q)=>{let H=Q.split("\t");return{id:H[0]||`unknown-${Date.now()}`,title:H[1]||"Untitled Event",location:H[2]||null,notes:H[3]||null,startDate:H[4]||null,endDate:H[5]||null,calendarName:H[6]||"Unknown Calendar",isAllDay:H[7]==="true",url:H[8]||null}})}async function R5(){try{return await f(`
...
4. Add your terminal/app to the allowed applications
5. Restart your terminal and try again`}}catch($){return{hasAccess:!1,message:`Error checking Calendar access: ${$ instanceof Error?$.message:String($)}`}}}async function m8(){let J=await f(`
tell application "Calendar"
    set calNames to {}
...
end tell`,D0.TIMEOUT_MS);if($.includes("non-existent")||$.includes("12345"))return{success:!1,message:"Event not found (test scenario)"};return{success:!0,message:H}}catch(J){return{success:!1,message:`Error opening event: ${J instanceof Error?J.message:String(J)}`}}}var D0,i4=`
on cleanField(theText)
    if theText is missing value then return ""
    set cleanedText to theText as text
    set {oldTID, AppleScript's text item delimiters} to {AppleScript's text item delimiters, return}
...
`)}`:`No events found from ${j} to ${G}.`}],isError:!1}}case"calendars":{let W=await H.getCalendarNames();return{content:[{type:"text",text:W.length>0?`Available calendars:

${W.join(`
`)}`:"No calendars found."}],isError:!1}}case"create":{let{title:W,startDate:X,endDate:w,location:z,notes:q,isAllDay:j,calendarName:G}=Q,U=await $0(m,"Create Calendar Event",`Title: ${W}
Start: ${X}
End: ${w}${z?`
Location: ${z}`:""}${G?`
