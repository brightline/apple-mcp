#!/usr/bin/env node
import{createRequire as A6}from"node:module";var O6=Object.create;var{getPrototypeOf:F6,defineProperty:X8,getOwnPropertyNames:S6}=Object;var k6=Object.prototype.hasOwnProperty;var M6=($,J,Q)=>{Q=$!=null?O6(F6($)):{};let H=J||!$||!$.__esModule?X8(Q,"default",{value:$,enumerable:!0}):Q;for(let Y of S6($))if(!k6.call(H,Y))X8(H,Y,{get:()=>$[Y],enumerable:!0});return H};var B8=($,J)=>()=>(J||$((J={exports:{}}).exports,J),J.exports);var F0=($,J)=>{for(var Q in J)X8($,Q,{get:J[Q],enumerable:!0,configurable:!0,set:(H)=>J[Q]=()=>H})};var W0=($,J)=>()=>($&&(J=$($=0)),J);var V4=A6(import.meta.url);import b$ from"node:process";import{promisify as f$}from"node:util";import{execFile as P$,execFileSync as OJ}from"node:child_process";async function p1($,{humanReadableOutput:J=!0}={}){if(b$.platform!=="darwin")throw Error("macOS only");let Q=J?[]:["-ss"],{stdout:H}=await C$("osascript",["-e",$,Q]);return H.trim()}var C$;var A8=W0(()=>{C$=f$(P$)});import{execFile as y$}from"node:child_process";import{promisify as Z$}from"node:util";import{writeFileSync as g$}from"node:fs";import{randomUUID as l$}from"node:crypto";import{tmpdir as m$}from"node:os";import{join as u$}from"node:path";function E($){return $.replace(/\0/g,"").replace(/\\/g,"\\\\").replace(/"/g,"\\\"").replace(/\r/g,"\\r").replace(/\n/g,"\\n").replace(/\t/g,"\\t")}function O1(){let $=process.env.HOME;if(!$)throw Error("HOME environment variable is not set");if(/[;&|`$(){}!\n\r]/.test($))throw Error("HOME path contains invalid characters");if(!$.startsWith("/Users/")&&!$.startsWith("/home/"))throw Error("HOME path does not resolve to a valid user directory");return $}function d1($,J){let Q=`${$}-${l$()}.txt`,H=u$(m$(),Q);return g$(H,J,{encoding:"utf8",mode:384}),H}async function b($,J){return Promise.race([p1($),new Promise((Q,H)=>setTimeout(()=>H(Error(`AppleScript timed out after ${J}ms. This usually means macOS has not granted the required app permission. Check System Settings > Privacy & Security > Automation.`)),J))])}async function F1($,J){let{stdout:Q}=await b8("sqlite3",["-json",$,J]);return Q}var b8;var U0=W0(()=>{A8();b8=Z$(y$)});var f8={};F0(f8,{default:()=>n$});async function c$(){try{return await b(`
tell application "Contacts"
    return name
end tell`,P0.TIMEOUT_MS),!0}catch($){return console.error(`Cannot access Contacts app: ${$ instanceof Error?$.message:String($)}`),!1}}async function i1(){try{if(await c$())return{hasAccess:!0,message:"Contacts access is already granted."};return{hasAccess:!1,message:`Contacts access is required but not granted. Please:
...
5. Restart your terminal and try again`}}catch($){return{hasAccess:!1,message:`Error checking Contacts access: ${$ instanceof Error?$.message:String($)}`}}}async function n1(){try{let $=await i1();if(!$.hasAccess)throw Error($.message);let J=`
tell application "Contacts"
    set contactList to {}
...
end tell`,Y=await b(H,P0.TIMEOUT_MS),W=Array.isArray(Y)?Y:Y?[Y]:[];if(W.length===0){console.error(`No AppleScript matches for "${$}", trying comprehensive search...`);let X=await n1(),w=(q)=>{return q.toLowerCase().replace(/[\u{1F600}-\u{1F64F}]|[\u{1F300}-\u{1F5FF}]|[\u{1F680}-\u{1F6FF}]|[\u{1F1E0}-\u{1F1FF}]|[\u{2600}-\u{26FF}]|[\u{2700}-\u{27BF}]/gu,"").replace(/[♥️❤️💙💚💛💜🧡🖤🤍🤎]/g,"").replace(/\s+/g," ").trim()},z=[(q)=>w(q)===Q,(q)=>{let V=w(q),K=w($);return V===K},(q)=>w(q).startsWith(Q),(q)=>w(q).includes(Q),(q)=>Q.includes(w(q)),(q)=>{let K=w(q).split(" ")[0];return K===Q||K.startsWith(Q)||Q.startsWith(K)||K.replace(/(.)\1+/g,"$1")===Q||Q.replace(/(.)\1+/g,"$1")===K},(q)=>{let K=w(q).split(" "),L=K[K.length-1];return L===Q||L.startsWith(Q)},(q)=>{return w(q).split(" ").some((L)=>L.includes(Q)||Q.includes(L)||L.replace(/(.)\1+/g,"$1")===Q)}];for(let q of z){let V=Object.keys(X).filter(q);if(V.length>0)return console.error(`Found ${V.length} matches using fuzzy strategy for "${$}": ${V.join(", ")}`),X[V[0]]||[]}}return W.filter((X)=>X&&X.trim()!=="")}catch(J){console.error(`Error finding contact: ${J instanceof Error?J.message:String(J)}`);try{let Q=await n1(),H=$.toLowerCase().trim(),Y=Object.keys(Q).find((W)=>W.toLowerCase().includes(H)||H.includes(W.toLowerCase()));if(Y)return console.error(`Fallback found match for "${$}": ${Y}`),Q[Y]}catch(Q){console.error(`Fallback search also failed: ${Q}`)}return[]}}async function d$($){try{let J=await i1();if(!J.hasAccess)throw Error(J.message);if(!$||$.trim()==="")return null;let Q=$.replace(/[^0-9+]/g,""),H=`
tell application "Contacts"
    set foundName to ""
    ...
    return foundName
end tell`,Y=await b(H,P0.TIMEOUT_MS);if(Y&&Y.trim()!=="")return Y;let W=await n1();for(let[X,w]of Object.entries(W))if(w.map((q)=>q.replace(/[^0-9+]/g,"")).some((q)=>q===Q||q===`+${Q}`||q===`+1${Q}`||`+1${q}`===Q||Q.includes(q)||q.includes(Q)))return X;return null}catch(J){return console.error(`Error finding contact by phone: ${J instanceof Error?J.message:String(J)}`),null}}var P0,n$;var P8=W0(()=>{U0();P0={MAX_CONTACTS:1000,TIMEOUT_MS:1e4};n$={getAllNumbers:n1,findNumber:p$,findContactByPhone:d$,requestContactsAccess:i1}});var E8={};F0(E8,{default:()=>$5});import{unlinkSync as i$}from"node:fs";async function o$(){try{return await b(`
tell application "Notes"
    return name
end tell`,l.TIMEOUT_MS),!0}catch($){return console.error(`Cannot access Notes app: ${$ instanceof Error?$.message:String($)}`),!1}}async function S1(){try{if(await o$())return{hasAccess:!0,message:"Notes access is already granted."};return{hasAccess:!1,message:`Notes access is required but not granted. Please:
1. Open System Settings > Privacy & Security > Automation
2. Find your terminal/app in the list and enable 'Notes'
3. Restart your terminal and try again
4. If the option is not available, run this command again to trigger the permission dialog`}}catch($){return{hasAccess:!1,message:`Error checking Notes access: ${$ instanceof Error?$.message:String($)}`}}}async function a$(){try{let $=await S1();if(!$.hasAccess)throw Error($.message);let J=`
tell application "Notes"
    set notesList to {}
    set noteCount to 0
    ...
    return notesList
end tell`,Q=await b(J,l.TIMEOUT_MS);return(Array.isArray(Q)?Q:Q?[Q]:[]).map((Y)=>({name:Y.name||"Untitled Note",content:Y.content||"",creationDate:void 0,modificationDate:void 0}))}catch($){return console.error(`Error getting all notes: ${$ instanceof Error?$.message:String($)}`),[]}}async function t$($){try{let J=await S1();if(!J.hasAccess)throw Error(J.message);if(!$||$.trim()==="")return[];let Q=$.toLowerCase(),H=`
tell application "Notes"
    set matchedNotes to {}
    set noteCount to 0
    ...
    return matchedNotes
end tell`,Y=await b(H,l.TIMEOUT_MS);return(Array.isArray(Y)?Y:Y?[Y]:[]).map((X)=>({name:X.name||"Untitled Note",content:X.content||"",creationDate:void 0,modificationDate:void 0}))}catch(J){return console.error(`Error finding notes: ${J instanceof Error?J.message:String(J)}`),[]}}async function r$($,J,Q="Claude"){try{let H=await S1();if(!H.hasAccess)return{success:!1,message:H.message};if(!$||$.trim()==="")return{success:!1,message:"Note title cannot be empty"};let Y=J.trim(),W=d1("note-content",Y),X=E(Q),w=E($),z=`
tell application "Notes"
...
end tell`,q=await b(z,l.TIMEOUT_MS);try{i$(W)}catch(V){}if(q&&typeof q==="string"&&q.startsWith("SUCCESS:")){let V=q.split(":"),K=V[1]||"Notes",L=V[2]==="true";return{success:!0,note:{name:$,content:Y},folderName:K,usedDefaultFolder:L}}else return{success:!1,message:`Failed to create note: ${q||"No result from AppleScript"}`}}catch(H){return{success:!1,message:`Failed to create note: ${H instanceof Error?H.message:String(H)}`}}}async function C8($){try{let J=await S1();if(!J.hasAccess)return{success:!1,message:J.message};let Q=`
tell application "Notes"
    set notesList to {}
    set noteCount to 0
    set folderFound to false
...
end tell`,H=await b(Q,l.TIMEOUT_MS);if(H&&typeof H==="string"){if(H.startsWith("ERROR:"))return{success:!1,message:H.replace("ERROR:","")};else if(H.startsWith("SUCCESS:"))return{success:!0,notes:[]}}return{success:!0,notes:[]}}catch(J){return{success:!1,message:`Failed to get notes from folder: ${J instanceof Error?J.message:String(J)}`}}}async function s$($,J=5){try{let Q=await C8($);if(Q.success&&Q.notes)return{success:!0,notes:Q.notes.slice(0,Math.min(J,Q.notes.length))};return Q}catch(Q){return{success:!1,message:`Failed to get recent notes from folder: ${Q instanceof Error?Q.message:String(Q)}`}}}async function e$($,J,Q,H=20){try{let Y=await C8($);if(Y.success&&Y.notes)return{success:!0,notes:Y.notes.slice(0,Math.min(H,Y.notes.length))};return Y}catch(Y){return{success:!1,message:`Failed to get notes by date range: ${Y instanceof Error?Y.message:String(Y)}`}}}var l,$5;var R8=W0(()=>{U0();l={MAX_NOTES:50,MAX_CONTENT_PREVIEW:200,TIMEOUT_MS:8000};$5={getAllNotes:a$,findNote:t$,createNote:r$,getNotesFromFolder:C8,getRecentNotesFromFolder:s$,getNotesByDateRange:e$,requestNotesAccess:S1}});var I8={};F0(I8,{default:()=>q5});import{access as J5}from"node:fs/promises";async function H5($){return new Promise((J)=>setTimeout(J,$))}async function v8($,J=Q5,Q=Y5){try{return await $()}catch(H){if(J>0)return console.error(`Operation failed, retrying... (${J} attempts remaining)`),await H5(Q),v8($,J-1,Q);throw H}}function W5($){let J=$.replace(/[^0-9+]/g,"");if(/^\+1\d{10}$/.test(J))return[J];if(/^1\d{10}$/.test(J))return[`+${J}`];if(/^\d{10}$/.test(J))return[`+1${J}`];let Q=new Set;if(J.startsWith("+1"))Q.add(J);else if(J.startsWith("1"))Q.add(`+${J}`);else Q.add(`+1${J}`);return Array.from(Q)}async function p4($,J){return await b(`
tell application "Messages"
    set targetService to 1st service whose service type = iMessage
    set targetBuddy to buddy "${E($)}"
    send "${E(J)}" to targetBuddy
end tell`,o1.TIMEOUT_MS)}async function X5(){try{let J=`${O1()}/Library/Messages/chat.db`;return await J5(J),await F1(J,"SELECT 1;"),!0}catch($){return console.error(`
...
`),!1}}async function N8(){try{if(await X5())return{hasAccess:!0,message:"Messages access is already granted."};try{return await b('tell application "Messages" to return name',o1.TIMEOUT_MS),{hasAccess:!1,message:`Messages app is accessible but database access is required. Please:
...
4. Restart your terminal and try again`}}}catch($){return{hasAccess:!1,message:`Error checking Messages access: ${$ instanceof Error?$.message:String($)}`}}}function d4($){try{let Q=Buffer.from($,"hex").toString(),H=[/NSString">(.*?)</,/NSString">([^<]+)/,/NSNumber">\d+<.*?NSString">(.*?)</,/NSArray">.*?NSString">(.*?)</,/"string":\s*"([^"]+)"/,/text[^>]*>(.*?)</,/message>(.*?)</],Y="";for(let w of H){let z=Q.match(w);if(z?.[1]){if(Y=z[1],Y.length>5)break}}let W=[/(https?:\/\/[^\s<"]+)/,/NSString">(https?:\/\/[^\s<"]+)/,/"url":\s*"(https?:\/\/[^"]+)"/,/link[^>]*>(https?:\/\/[^<]+)/],X;for(let w of W){let z=Q.match(w);if(z?.[1]){X=z[1];break}}if(!Y&&!X){let w=Q.replace(/streamtyped.*?NSString/g,"").replace(/NSAttributedString.*?NSString/g,"").replace(/NSDictionary.*?$/g,"").replace(/\+[A-Za-z]+\s/g,"").replace(/NSNumber.*?NSValue.*?\*/g,"").replace(/[^\x20-\x7E]/g," ").replace(/\s+/g," ").trim();if(w.length>5)Y=w;else return{text:"[Message content not readable]"}}if(Y)Y=Y.replace(/^[+\s]+/,"").replace(/\s*iI\s*[A-Z]\s*$/,"").replace(/\s+/g," ").trim();return{text:Y||X||"",url:X}}catch(J){return console.error("Error decoding attributedBody:",J),{text:"[Message content not readable]"}}}async function n4($){try{let J=`
        ...
        `,Q=O1(),H=await F1(`${Q}/Library/Messages/chat.db`,J);if(!H.trim())return[];return JSON.parse(H).map((W)=>W.filename).filter(Boolean)}catch(J){return console.error("Error getting attachments:",J),[]}}async function B5($,J=10){try{let Q=Math.min(J,o1.MAX_MESSAGES),H=await N8();if(!H.hasAccess)throw Error(H.message);let Y=W5($);console.error("Trying phone formats:",Y);let X=`
            ...
            WHERE h.id IN (${Y.map((K)=>`'${K.replace(/'/g,"''")}'`).join(",")})
            ...
            LIMIT ${Q}
        `,w=O1(),z=await v8(()=>F1(`${w}/Library/Messages/chat.db`,X));if(!z.trim())return console.error("No messages found in database for the given phone number"),[];let q=JSON.parse(z);return await Promise.all(q.filter((K)=>K.content!==null||K.cache_has_attachments===1).map(async(K)=>{let L=K.content||"",P;if(K.content_type===1){let i=d4(L);L=i.text,P=i.url}else{let i=L.match(/(https?:\/\/[^\s]+)/);if(i)P=i[1]}let h=[];if(K.cache_has_attachments)h=await n4(K.message_id);if(K.subject)L=`Subject: ${K.subject}
${L}`;let y={content:L||"[No text content]",date:new Date(K.date).toISOString(),sender:K.sender,is_from_me:Boolean(K.is_from_me)};if(h.length>0)y.attachments=h,y.content+=`
[Attachments: ${h.length}]`;if(P)y.url=P,y.content+=`
[URL: ${P}]`;return y}))}catch(Q){if(console.error("Error reading messages:",Q),Q instanceof Error)console.error("Error details:",Q.message),console.error("Stack trace:",Q.stack);return[]}}async function w5($=10){try{let J=Math.min($,o1.MAX_MESSAGES),Q=await N8();if(!Q.hasAccess)throw Error(Q.message);let H=`
            SELECT 
                m.ROWID as message_id,
                CASE 
                    WHEN m.text IS NOT NULL AND m.text != '' THEN m.text
            ...
            ORDER BY m.date DESC 
            LIMIT ${J}
        `,Y=O1(),W=await v8(()=>F1(`${Y}/Library/Messages/chat.db`,H));if(!W.trim())return console.error("No unread messages found"),[];let X=JSON.parse(W);return await Promise.all(X.filter((z)=>z.content!==null||z.cache_has_attachments===1).map(async(z)=>{let q=z.content||"",V;if(z.content_type===1){let P=d4(q);q=P.text,V=P.url}else{let P=q.match(/(https?:\/\/[^\s]+)/);if(P)V=P[1]}let K=[];if(z.cache_has_attachments)K=await n4(z.message_id);if(z.subject)q=`Subject: ${z.subject}
${q}`;let L={content:q||"[No text content]",date:new Date(z.date).toISOString(),sender:z.sender,is_from_me:Boolean(z.is_from_me)};if(K.length>0)L.attachments=K,L.content+=`
[Attachments: ${K.length}]`;if(V)L.url=V,L.content+=`
[URL: ${V}]`;return L}))}catch(J){if(console.error("Error reading unread messages:",J),J instanceof Error)console.error("Error details:",J.message),console.error("Stack trace:",J.stack);return[]}}async function z5($,J,Q){let H=new Map,Y=Q.getTime()-Date.now();if(Y<0)throw Error("Cannot schedule message in the past");let W=setTimeout(async()=>{try{await p4($,J),H.delete(W)}catch(X){console.error("Failed to send scheduled message:",X)}},Y);return H.set(W,{phoneNumber:$,message:J,scheduledTime:Q,timeoutId:W}),{id:W,scheduledTime:Q,message:J,phoneNumber:$}}var o1,Q5=3,Y5=1000,q5;var h8=W0(()=>{U0();o1={MAX_MESSAGES:50,MAX_CONTENT_PREVIEW:300,TIMEOUT_MS:8000};q5={sendMessage:p4,readMessages:B5,scheduleMessage:z5,getUnreadMessages:w5,requestMessagesAccess:N8}});var y8={};F0(y8,{default:()=>F5});import{unlinkSync as G5}from"node:fs";function T8($){if(!$||$.trim()==="")return[];return $.split(/\r?\n/).filter((Q)=>Q.includes("\t")).map((Q)=>{let H=Q.split("\t");return{subject:H[0]||"No subject",sender:H[1]||"Unknown",dateSent:H[2]||"",content:H[3]||"",isRead:H[4]==="true",mailbox:H[5]||"Unknown"}})}async function j5(){try{return await b(`
tell application "Mail"
    return name
end tell`,x.TIMEOUT_MS),!0}catch($){return console.error(`Cannot access Mail app: ${$ instanceof Error?$.message:String($)}`),!1}}async function _0(){try{if(await j5())return{hasAccess:!0,message:"Mail access is already granted."};return{hasAccess:!1,message:`Mail access is required but not granted. Please:
1. Open System Settings > Privacy & Security > Automation
...
4. Restart your terminal and try again`}}catch($){return{hasAccess:!1,message:`Error checking Mail access: ${$ instanceof Error?$.message:String($)}`}}}async function K5($=10){try{let J=await _0();if(!J.hasAccess)throw Error(J.message);let Q=Math.min($,x.MAX_EMAILS),H=`
tell application "Mail"
	set outputText to ""
	set emailCount to 0
...
${x8}`,Y=await b(H,x.TIMEOUT_MS);return T8(Y)}catch(J){return console.error(`Error getting unread emails: ${J instanceof Error?J.message:String(J)}`),[]}}async function V5($,J=10){try{let Q=await _0();if(!Q.hasAccess)throw Error(Q.message);if(!$||$.trim()==="")return[];let H=Math.min(J,x.MAX_EMAILS),W=`
tell application "Mail"
	set outputText to ""
	set emailCount to 0
	set searchTerm to "${E($)}"
...
${x8}`,X=await b(W,x.TIMEOUT_MS);return T8(X)}catch(Q){return console.error(`Error searching emails: ${Q instanceof Error?Q.message:String(Q)}`),[]}}async function U5($,J,Q,H,Y){try{let W=await _0();if(!W.hasAccess)throw Error(W.message);if(!$||!$.trim())throw Error("To address is required");if(!J||!J.trim())throw Error("Subject is required");if(!Q||!Q.trim())throw Error("Email body is required");let X=d1("email-body",Q.trim()),w=`
tell application "Mail"
    activate
    ...
    send newMessage
    return "SUCCESS"
end tell`,z=await b(w,x.TIMEOUT_MS);try{G5(X)}catch(q){}if(z==="SUCCESS")return`Email sent to ${$} with subject "${J}"`;else throw Error("Failed to send email")}catch(W){throw console.error(`Error sending email: ${W instanceof Error?W.message:String(W)}`),Error(`Error sending email: ${W instanceof Error?W.message:String(W)}`)}}async function _5(){try{let $=await _0();if(!$.hasAccess)throw Error($.message);let Q=await b(`
tell application "Mail"
	set boxNames to {}
	repeat with mb in mailboxes
		try
	...
	return outputText
end tell`,x.TIMEOUT_MS);if(Q&&Q.trim())return Q.split(/\r?\n/).filter((H)=>H.trim()!=="");return[]}catch($){return console.error(`Error getting mailboxes: ${$ instanceof Error?$.message:String($)}`),[]}}async function D5(){try{let $=await _0();if(!$.hasAccess)throw Error($.message);let Q=await b(`
tell application "Mail"
	set acctNames to {}
	...
	set AppleScript's text item delimiters to oldTID
	return outputText
end tell`,x.TIMEOUT_MS);if(Q&&Q.trim())return Q.split(/\r?\n/).filter((H)=>H.trim()!=="");return[]}catch($){return console.error(`Error getting accounts: ${$ instanceof Error?$.message:String($)}`),[]}}async function L5($){try{let J=await _0();if(!J.hasAccess)throw Error(J.message);if(!$||!$.trim())return[];let Q=`
tell application "Mail"
	set boxNames to {}
	...
	set AppleScript's text item delimiters to oldTID
	return outputText
end tell`,H=await b(Q,x.TIMEOUT_MS);if(H&&H.trim())return H.split(/\r?\n/).filter((Y)=>Y.trim()!=="");return[]}catch(J){return console.error(`Error getting mailboxes for account: ${J instanceof Error?J.message:String(J)}`),[]}}async function O5($,J=5){try{let Q=await _0();if(!Q.hasAccess)throw Error(Q.message);let H=Math.min(J,x.MAX_EMAILS),Y=`
tell application "Mail"
	set outputText to ""
	set emailCount to 0
...
${x8}`,W=await b(Y,x.TIMEOUT_MS);if(W&&W.startsWith("ERROR:"))throw Error(W.substring(6));return T8(W)}catch(Q){return console.error("Error getting latest emails:",Q),[]}}var x,x8=`
on cleanField(theText)
	...
	set AppleScript's text item delimiters to oldTID
	return cleanedText
end cleanField`,F5;var Z8=W0(()=>{U0();x={MAX_EMAILS:20,MAX_CONTENT_PREVIEW:300,TIMEOUT_MS:1e4};F5={getUnreadMails:K5,searchMails:V5,sendMail:U5,getMailboxes:_5,getAccounts:D5,getMailboxesForAccount:L5,getLatestMails:O5,requestMailAccess:_0}});var g8={};F0(g8,{default:()=>P5});async function S5(){try{return await b(`
tell application "Reminders"
    return name
end tell`,D0.TIMEOUT_MS),!0}catch($){return console.error(`Cannot access Reminders app: ${$ instanceof Error?$.message:String($)}`),!1}}async function C0(){try{if(await S5())return{hasAccess:!0,message:"Reminders access is already granted."};return{hasAccess:!1,message:`Reminders access is required but not granted. Please:
1. Open System Settings > Privacy & Security > Automation
2. Find your terminal/app in the list and enable 'Reminders'
3. Restart your terminal and try again
4. If the option is not available, run this command again to trigger the permission dialog`}}catch($){return{hasAccess:!1,message:`Error checking Reminders access: ${$ instanceof Error?$.message:String($)}`}}}async function k5(){try{let $=await C0();if(!$.hasAccess)throw Error($.message);let J=`
tell application "Reminders"
    set listArray to {}
    set listCount to 0
    ...
    repeat with i from 1 to (count of allLists)
        if listCount >= ${D0.MAX_LISTS} then exit repeat
    ...
    return listArray
end tell`,Q=await b(J,D0.TIMEOUT_MS);return(Array.isArray(Q)?Q:Q?[Q]:[]).map((Y)=>({name:Y.name||"Untitled List",id:Y.id||"unknown-id"}))}catch($){return console.error(`Error getting reminder lists: ${$ instanceof Error?$.message:String($)}`),[]}}async function M5($){try{let J=await C0();if(!J.hasAccess)throw Error(J.message);let H=await b(`
tell application "Reminders"
    try
...
end tell`,D0.TIMEOUT_MS);if(H&&typeof H==="string"&&H.includes("SUCCESS"))return[];return[]}catch(J){return console.error(`Error getting reminders: ${J instanceof Error?J.message:String(J)}`),[]}}async function i4($){try{let J=await C0();if(!J.hasAccess)throw Error(J.message);if(!$||$.trim()==="")return[];let H=await b(`
tell application "Reminders"
    try
...
end tell`,D0.TIMEOUT_MS);return[]}catch(J){return console.error(`Error searching reminders: ${J instanceof Error?J.message:String(J)}`),[]}}async function A5($,J="Reminders",Q,H){try{let Y=await C0();if(!Y.hasAccess)throw Error(Y.message);if(!$||$.trim()==="")throw Error("Reminder name cannot be empty");let W=E($),X=E(J),w=Q?E(Q):"",z=`
tell application "Reminders"
    try
...
end tell`,q=await b(z,D0.TIMEOUT_MS);if(q&&q.startsWith("SUCCESS:")){let V=q.replace("SUCCESS:","");return{name:$,id:"created-reminder-id",body:Q||"",completed:!1,dueDate:H||null,listName:V}}else throw Error(`Failed to create reminder: ${q}`)}catch(Y){throw Error(`Failed to create reminder: ${Y instanceof Error?Y.message:String(Y)}`)}}async function b5($){try{let J=await C0();if(!J.hasAccess)return{success:!1,message:J.message};let Q=await i4($);if(Q.length===0)return{success:!1,message:"No matching reminders found"};if(await b(`
tell application "Reminders"
    activate
    return "SUCCESS"
end tell`,D0.TIMEOUT_MS)==="SUCCESS")return{success:!0,message:"Reminders app opened",reminder:Q[0]};else return{success:!1,message:"Failed to open Reminders app"}}catch(J){return{success:!1,message:`Failed to open reminder: ${J instanceof Error?J.message:String(J)}`}}}async function f5($,J){try{let Q=await C0();if(!Q.hasAccess)throw Error(Q.message);let Y=await b(`
tell application "Reminders"
    try
...
end tell`,D0.TIMEOUT_MS);return[]}catch(Q){return console.error(`Error getting reminders from list by ID: ${Q instanceof Error?Q.message:String(Q)}`),[]}}var D0,P5;var l8=W0(()=>{U0();D0={MAX_REMINDERS:50,MAX_LISTS:20,TIMEOUT_MS:8000};P5={getAllLists:k5,getAllReminders:M5,searchReminders:i4,createReminder:A5,openReminder:b5,getRemindersFromListById:f5,requestRemindersAccess:C0}});var c8={};F0(c8,{default:()=>l5});import{dirname as C5,join as E5}from"node:path";import{fileURLToPath as R5}from"node:url";async function m8($){let{stdout:J,stderr:Q}=await b8("swift",[N5,...$],{timeout:o0.SWIFT_TIMEOUT_MS});if(Q)console.error(`Swift helper stderr: ${Q}`);return J}async function I5(){try{return await b(`
tell application "Calendar"
    return name
end tell`,o0.TIMEOUT_MS),!0}catch($){return console.error(`Cannot access Calendar app: ${$ instanceof Error?$.message:String($)}`),!1}}async function u8(){try{if(await I5())return{hasAccess:!0,message:"Calendar access is already granted."};return{hasAccess:!1,message:`Calendar access is required but not granted. Please:
1. Open System Settings > Privacy & Security > Automation
2. Find your terminal/app in the list and enable 'Calendar'
3. Alternatively, open System Settings > Privacy & Security > Calendars
4. Add your terminal/app to the allowed applications
5. Restart your terminal and try again`}}catch($){return{hasAccess:!1,message:`Error checking Calendar access: ${$ instanceof Error?$.message:String($)}`}}}async function h5(){try{let $=await m8(["list-calendars"]);return JSON.parse($).map((Q)=>Q.name)}catch($){return console.error(`Error getting calendar names: ${$ instanceof Error?$.message:String($)}`),[]}}async function x5($=10,J,Q,H){try{console.error("getEvents - Starting to fetch calendar events via EventKit");let Y=Math.min($,o0.MAX_EVENTS),W=["list-events","--limit",String(Y)];if(J)W.push("--from",J);if(Q)W.push("--to",Q);if(H)W.push("--calendar",H);let X=await m8(W),w=JSON.parse(X);return console.error(`getEvents - Found ${w.length} event(s)`),w}catch(Y){return console.error(`Error getting events: ${Y instanceof Error?Y.message:String(Y)}`),[]}}async function T5($,J=10,Q,H,Y){try{console.error(`searchEvents - Searching for: "${$}" via EventKit`);let W=Math.min(J,o0.MAX_EVENTS),X=["search-events","--query",$,"--limit",String(W)];if(Q)X.push("--from",Q);if(H)X.push("--to",H);if(Y)X.push("--calendar",Y);let w=await m8(X),z=JSON.parse(w);return console.error(`searchEvents - Found ${z.length} matching event(s)`),z}catch(W){return console.error(`Error searching events: ${W instanceof Error?W.message:String(W)}`),[]}}async function y5($,J,Q,H,Y,W=!1,X){try{let w=await u8();if(!w.hasAccess)return{success:!1,message:w.message};if(!$.trim())return{success:!1,message:"Event title cannot be empty"};if(!J||!Q)return{success:!1,message:"Start date and end date are required"};let z=new Date(J),q=new Date(Q);if(isNaN(z.getTime())||isNaN(q.getTime()))return{success:!1,message:"Invalid date format. Please use ISO format (YYYY-MM-DDTHH:mm:ss.sssZ)"};if(q<=z)return{success:!1,message:"End date must be after start date"};console.error(`createEvent - Attempting to create event: "${$}"`);let V=X||"Calendar",K=`
tell application "Calendar"
...
tell application "Mail"
    ...
    return resultList
end tell`;try{let X=await p1(W);if(X&&X.startsWith("Error:"))throw Error(X);let w=[],z=X.match(/\{([^}]+)\}/g);if(z&&z.length>0)for(let q of z)try{let V=q.substring(1,q.length-1).split(","),K={};if(V.forEach((L)=>{let P=L.split(":");if(P.length>=2){let h=P[0].trim(),y=P.slice(1).join(":").trim();K[h]=y}}),K.subject||K.sender)w.push({subject:K.subject||"No subject",sender:K.sender||"Unknown sender",dateSent:K.date||new Date().toString(),content:K.content||"[Content not available]",isRead:!1,mailbox:`${Q.account} - ${K.mailbox||"Unknown"}`})}catch(V){console.error("Error parsing email match:",V)}Y=w}catch(X){console.error("Error getting account-specific emails:",X),Y=await H.getUnreadMails(Q.limit)}}else Y=await H.getUnreadMails(Q.limit);return{content:[{type:"text",text:Y.length>0?`Found ${Y.length} unread email(s)${Q.account?` in account "${Q.account}"`:""}${Q.mailbox?` and mailbox "${Q.mailbox}"`:""}:
...
`):`No latest emails found in account "${Y}"`}],isError:!1}}default:throw Error(`Unknown operation: ${Q.operation}`)}}catch(H){let Y=H instanceof Error?H.message:String(H);return{content:[{type:"text",text:Y.includes("access")?Y:`Error with mail operation: ${Y}`}],isError:!0}}}case"reminders":{if(!d7(Q))throw Error("Invalid arguments for reminders tool");try{let H=await O0("reminders"),{operation:Y}=Q;if(Y==="list"){let W=await H.getAllLists(),X=await H.getAllReminders();return{content:[{type:"text",text:`Found ${W.length} lists and ${X.length} reminders.`}],lists:W,reminders:X,isError:!1}}else if(Y==="search"){let{searchText:W}=Q,X=await H.searchReminders(W);return{content:[{type:"text",text:X.length>0?`Found ${X.length} reminders matching "${W}".`:`No reminders found matching "${W}".`}],reminders:X,isError:!1}}else if(Y==="open"){let{searchText:W}=Q,X=await H.openReminder(W);return{content:[{type:"text",text:X.success?`Opened Reminders app. Found reminder: ${X.reminder?.name}`:X.message}],...X,isError:!X.success}}else if(Y==="create"){let{name:W,listName:X,notes:w,dueDate:z}=Q,q=await $0(m,"Create Reminder",`Name: ${W}${X?`
List: ${X}`:""}${z?`
Due: ${z}`:""}`);if(!q.confirmed)return{content:[{type:"text",text:q.message}],isError:!0};let V=await H.createReminder(W,X,w,z);return{content:[{type:"text",text:`Created reminder "${V.name}" ${X?`in list "${X}"`:""}.`}],success:!0,reminder:V,isError:!1}}else if(Y==="listById"){let{listId:W,props:X}=Q,w=await H.getRemindersFromListById(W,X);return{content:[{type:"text",text:w.length>0?`Found ${w.length} reminders in list with ID "${W}".`:`No reminders found in list with ID "${W}".`}],reminders:w,isError:!1}}return{content:[{type:"text",text:"Unknown operation"}],isError:!0}}catch(H){console.error("Error in reminders tool:",H);let Y=H instanceof Error?H.message:String(H);return{content:[{type:"text",text:Y.includes("access")?Y:`Error in reminders tool: ${Y}`}],isError:!0}}}case"calendar":{if(!n7(Q))throw Error("Invalid arguments for calendar tool");try{let H=await O0("calendar"),{operation:Y}=Q;switch(Y){case"search":{let{searchText:W,limit:X,fromDate:w,toDate:z,calendarName:q}=Q,V=await H.searchEvents(W,X,w,z,q);return{content:[{type:"text",text:V.length>0?`Found ${V.length} events matching "${W}":

${V.map((K)=>`${K.title} (${new Date(K.startDate).toLocaleString()} - ${new Date(K.endDate).toLocaleString()})
...
`)}`:`No events found matching "${W}".`}],isError:!1}}case"open":{let{eventId:W}=Q,X=await H.openEvent(W);return{content:[{type:"text",text:X.success?X.message:`Error opening event: ${X.message}`}],isError:!X.success}}case"list":{let{limit:W,fromDate:X,toDate:w,calendarName:z}=Q,q=await H.getEvents(W,X,w,z),V=X?new Date(X).toLocaleDateString():"today",K=w?new Date(w).toLocaleDateString():"next 7 days";return{content:[{type:"text",text:q.length>0?`Found ${q.length} events from ${V} to ${K}:
...
`)}`:`No events found from ${V} to ${K}.`}],isError:!1}}case"calendars":{let W=await H.getCalendarNames();return{content:[{type:"text",text:W.length>0?`Available calendars:

${W.join(`
`)}`:"No calendars found."}],isError:!1}}case"create":{let{title:W,startDate:X,endDate:w,location:z,notes:q,isAllDay:V,calendarName:K}=Q,L=await $0(m,"Create Calendar Event",`Title: ${W}
Start: ${X}
End: ${w}${z?`
Location: ${z}`:""}${K?`
Calendar: ${K}`:""}`);if(!L.confirmed)return{content:[{type:"text",text:L.message}],isError:!0};let P=await H.createEvent(W,X,w,z,q,V,K);return{content:[{type:"text",text:P.success?`${P.message} Event scheduled from ${new Date(X).toLocaleString()} to ${new Date(w).toLocaleString()}${P.eventId?`
Event ID: ${P.eventId}`:""}`:`Error creating event: ${P.message}`}],isError:!P.success}}default:throw Error(`Unknown calendar operation: ${Y}`)}}catch(H){let Y=H instanceof Error?H.message:String(H);return{content:[{type:"text",text:Y.includes("access")?Y:`Error in calendar tool: ${Y}`}],isError:!0}}}case"maps":{if(!i7(Q))throw Error("Invalid arguments for maps tool");try{let H=await O0("maps"),{operation:Y}=Q;switch(Y){case"search":{let{query:W,limit:X}=Q;if(!W)throw Error("Search query is required for search operation");let w=await H.searchLocations(W,X);return{content:[{type:"text",text:w.success?`${w.message}
